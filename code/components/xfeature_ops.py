            ...
            for inst_type in os.listdir(hit_type_dir):
                ...
                for audio_name in os.listdir(inst_type_dir):
                    ...
                    if as_average["spec_bw"]: # boolean, global
                        ...
                        features["spec_bw"] = spec_bw
                    # print("spec_bw shape: ", end="")
                    # print(features["spec_bw"].shape)
                    # print("spec_bw:")
                    # print(features["spec_bw"])
                    ...
                    """ array of an array of numbers, get the average """
                    # truncate mfcc

                    # print("mfcc old shape: ", end="")
                    # print(mfcc.shape) # this is (20, 112)

                    mfcc = numpy.resize(mfcc, (20, cutoff))
                        # used to be (20, 13)
                    # print("mfcc new shape: ", end="")
                    # print(mfcc.shape)
                    # print("mfcc...")
                    # print(mfcc)
                    # print(mfcc.shape)
                    ...
                    /58610/mfcc-classification-model
                    """
                    ...
                    # Sample code, unused
                    # featureVector = numpy.empty(13+5) # 13 + 5 columns
                    resized_vect = numpy.resize(spec_bw, (1,6))
                        # This is how to resize a numpy array
                        # resized_arr = numpy.resize(arr, (row, col))
                    ...
                    # print(features) # printing dictionary items
                    ...
                    # for instanciating a numpy array
                    # creating the 1-dimensional feature array in 
                    # inst_feat_arr = numpy.empty(feat_arr_len)
                    # inst_feat_arr = numpy.zeros(10)
                    # feat_arr = numpy.arange(20) # initialize with values 1-20
                    # feat_arr = numpy.resize(self.features, (0,13+5))

                    audio_feat_list = [] # initialize here for reference (?)

                    # inst_feat_arr = numpy.arange(feat_arr_len)
                    # inst_feat_arr = numpy.append(inst_feat_arr \
                        # , features["mfcc"])

                    # for feat in audio_features:
                        # inst_feat_arr 
                        #   = numpy.vstack([inst_feat_arr, features[feat]])
...
def import_features(type=None, blacklist=None, whitelist=None):
    ...
    if blacklist != None:
        ...
        for scp in scope:
            ...
            scp_split.sort()
            print("scp_sorted: {}".format(scp_split))
        ...
        for black in blacklist:
            ...
            blk_sort = blk_split.sort()
            black_split.append(blk_sort)


        blacklist_names = []

        for inst_str in blacklist:
            insts = inst_str.split()
            insts_len = len(insts)
...
def get_inst_comb(instruments):
    ...
    if len(single) >= 4:
        ...
        if "bass" in single and "hihat" in single:
            for insts in triple:
                if "bass" in insts.split() or "hihat" in insts.split():
                    break
                else:
